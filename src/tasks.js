...
export function taskAction(state, action) {
  if (!state.projects.some((p) => p.id === action.projectId))
    throw Error("הפרויקט אינו קיים. יש לרענן את הרשימה.");
  if (!["add", "rename", "complete", "delete"].includes(action.type))
    throw Error("פעולת משימה לא מוכרת.");
  if (!validId(action.id)) throw Error("מזהה המשימה אינו תקין.");
  if ((action.type === "add" || action.type === "rename") && !validTitle(action.title))
    throw Error("יש להזין משימה באורך של עד 300 תווים.");
  if (action.type === "complete" && typeof action.completed !== "boolean")
    throw Error("מצב המשימה אינו תקין.");
  state.tasks ??= [];
  const task = state.tasks.find((t) => t.id === action.id);
  if (action.type === "add") {
    if (task) {
      if (task.projectId !== action.projectId) throw Error("מזהה המשימה כבר בשימוש.");
      return;
    }
    if (state.tasks.length >= 99999) throw Error("רשימת המשימות מלאה.");
    state.tasks.push({ id: action.id, projectId: action.projectId, title: action.title.trim(), completed: false });
    return;
  }
  if (!task) {
    if (action.type === "delete") return;
    throw Error("המשימה נמחקה בחלון אחר. יש לרענן את הרשימה.");
  }
  if (task.projectId !== action.projectId) throw Error("המשימה שייכת לפרויקט אחר.");
  if (action.type === "rename") {
    if (task.title !== action.expectedTitle)
      throw Error("המשימה נערכה בחלון אחר. בטל את העריכה ופתח אותה מחדש.");
